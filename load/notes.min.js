$(function(){loginsys()});var userinformation={"name":null,"id":null,"admin":!1};var errorMessageTimeOut=null;function review(enabled){$("div.login").addClass("disable");$("div.noteview").addClass("disable");$("div.noteslist").addClass("disable");$("div."+enabled).removeClass("disable")}
...
var cm_editor;function maker(noteid,notename){review("noteview");var notedata;var noteOverrideDanger=!1;function get_notedata(){function getDataWithoutServer(empty){if(!empty){noteOverrideDanger=!0;errorMessage('Kann die aktuelle Version der Notiz nicht vom Server holen.',20)}
...
$("div.noteview div.loading").removeClass("disable");ajax_request("view",{"userid":userinformation.id,"noteid":noteid},function(data){$("div.noteview div.loading").addClass("disable");if(data.status==='okay'){if(!data.data.empty){notedata={"name":data.data.name,"id":data.data.id,"content":data.data.content};make_inputfield()}
else{getDataWithoutServer(!0)}}
else{getDataWithoutServer(!1)}},function(data){getDataWithoutServer(!1)})}
function closebutton(){$("button#closenote").unbind("click").click(function(){cm_editor.off("change",here_parser_reparse);cm_editor.off("change",here_codemi_save);ajaxsave(function(okay){if(okay){list()}
else{$("body").append('<div id="errorMessageNoteSave">Die Speicherung der Notiz auf dem Server schlug fehl!<br />Wollen Sie den Editor verlassen und einen Verlust der Änderungen in Kauf nehmen oder abbrechen?</div>');$("#errorMessageNoteSave").dialog({resizable:!1,height:"auto",width:"auto",modal:!0,title:'Fehler beim Speichern!',buttons:{"Änderungen verwerfen":function(){$(this).dialog("close");list()},"Abbrechen":function(){$(this).dialog("close")}},close:function(){$(this).remove()}})}})});$("button#publishnote").unbind('click').click(freigabeManager);$("button#notehistory").unbind('click').click(historyManager)}
...
function doSave(){$("div.noteview div.loading").removeClass("disable");ajax_request("view",{"userid":userinformation.id,"noteid":noteid,"note":{"name":$("input#notename").val(),"cont":cm_editor.getValue()}},function(data){$("div.noteview div.loading").addClass("disable");if(data.status==='okay'){console.log('Notiz: "'+notename+'" ("'+noteid+'") auf Server gespeichert.');lastajaxsave=Date.now()}
if(typeof callback==="function"){callback((data.status==='okay'))}},function(data){if(typeof callback==="function"){callback(!1)}})}}
get_notedata();closebutton();function freigabeManager(){alert('\n!!!Funktion noch nicht vorhanden!!!\n')}
function historyManager(){alert('\n!!!Funktion noch nicht vorhanden!!!\n')}}
function authCodeManager(){function initDialog(){$("body").append('<div id="authCodeManagerDialog">Lädt ...</div>');$("div#authCodeManagerDialog").dialog({resizable:!1,height:"auto",width:"auto",modal:!0,title:'Authentifizierungslinks und Passwort',close:function(){$(this).remove()}})}
function dialogSetContent(cont){$("div#authCodeManagerDialog").html(cont)}
function loadContent(authcodes){var html='<h3>Authentifizierungslinks</h3>'
if(authcodes!==!1){html+='<table>'+'<tr>'+'<th>Code (Anfang)</th>'+'<th>Letzte Nutzung</th>'+'<th>Löschen</th>'+'</tr>';authcodes.forEach(function(value){html+='<tr>'+'<td><code style="color:black;">'+value.code+'</code></td>'+'<td>'+value.time+'</td>'+'<td><button class="deleteAuthLink" linkid="'+value.id+'">Link Löschen</button></td>'+'</tr>'});html+='</table>'}
...
else{$("button#newPasswordSet").prop('disabled',!0)}});$("button.deleteAuthLink").click(function(){var codeid=$(this).attr('linkid');alert('Löschen noch nicht möglich!\n\n'+codeid)});$("button#addAuthLink").click(function(){alert('Hinzufügen noch nicht möglich!')});function testPasswords(pA,pB){var error=!1;$("span#newPasswordAIndikator").css({'color':'white'});if(pA==""){$("span#newPasswordAIndikator").css({'background-color':'inherit'});$("span#newPasswordAIndikator").text('Bitte geben Sie ein Passwort ein!');error=!0}
...
initDialog();ajax_request("account",{userid:userinformation.id,art:'list',id:'list'},function(data){if(data.status==='okay'){loadContent(data.data)}
else{dialogSetContent('Kann nicht laden!')}})}
function oldNotesManager(){alert('\n!!!Funktion noch nicht vorhanden!!!\n')}