...
function ajax_request(task,post,callback){$.post(domain+"/ajax.php?"+task,post,function(data){if(typeof data==="object"){if(data.status==='error'){console.log(data.error)}
if(typeof callback==="function"){callback(data)}}
else{errorMessage('Sever antwortet nicht korrekt!',!1)}}).fail(function(){errorMessage('Verbindung zum Sever verloren!',!1)})}
...
function ajaxsave(callback){$("div.noteview div.loading").removeClass("disable");ajax_request("view",{"userid":userinformation.id,"noteid":noteid,"note":{"name":$("input#notename").val(),"cont":cm_editor.getValue()}},function(data){$("div.noteview div.loading").addClass("disable");if(data.status==='okay'){console.log('Notiz: "'+notename+'" ("'+noteid+'") auf Server gespeichert.');lastajaxsave=Date.now()}
if(typeof callback==="function"){callback((data.status==='okay'))}})}
get_notedata();closebutton();function freigabeManager(){alert('\n!!!Funktion noch nicht vorhanden!!!\n')}
function historyManager(){alert('\n!!!Funktion noch nicht vorhanden!!!\n')}}
function authCodeManager(){alert('Hallo '+userinformation.name+'!\n\n!!!Funktion noch nicht vorhanden!!!\n- AuthCodes verwalten\n- Passwort ändern')}
